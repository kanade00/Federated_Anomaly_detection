...
def get_neg_features(model, trainset, dataloader, shift_trans_type=None):
    features, index_array = [], []

    if shift_trans_type == 'rotation':
        shift_transform = TL.Rotation()
        numpy_transform = rotate_np
        num_transform = 4
    elif shift_trans_type == 'cutperm':
        shift_transform = TL.CutPerm()
        numpy_transform = cutperm_np
        num_transform = 4
    else:
        raise Exception

    image_array = trainset.data
    neg_imgs = np.concatenate([numpy_transform(image_array, k) for k in range(1, num_transform)])
    neg_imgs = np.split(neg_imgs, num_transform - 1, axis=0)
    neg_imgs = np.concatenate([np.expand_dims(neg_img, axis=1) for neg_img in neg_imgs], axis=1)
    ...
    for index, data in enumerate(dataloader):
        ...
        outputs = model(imgs)
        outputs_list = torch.split(outputs, [bsz] * (num_transform - 1), dim=0)
        outputs = torch.cat([outputs.unsqueeze(1) for outputs in outputs_list], dim=1)

        features += list(outputs.data.cpu().numpy())
        index_array += list(index.data.cpu().numpy())

    features = np.array(features)
    features[index_array] = features

    return neg_imgs, features


def get_aligned_features(model, dataloader):
    features, labels, index_array = [], [], []
...
def get_fpr(xin, xood):
    return np.sum(xood < np.percentile(xin, 90)) / len(xood)
...
def cutperm_np(inputs, aug_index):
    ...
    if jigsaw_h == 1:
        inputs = np.concatenate((inputs[:, :, h_mid:, :], inputs[:, :, 0:h_mid, :]), axis=2)
    if jigsaw_v == 1:
        inputs = np.concatenate((inputs[:, :, :, w_mid:], inputs[:, :, :, 0:w_mid]), axis=3)
...
def rotate_np(inputs, aug_index):
    inputs = inputs.transpose((0, 3, 1, 2))
    outputs = np.rot90(inputs, aug_index, (2, 3))
