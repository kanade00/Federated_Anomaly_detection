...
class RandomResizedCropLayer(nn.Module):
    ...
    def forward(self, inputs, whbias=None):
        ...
        grid = F.affine_grid(_theta, inputs.size()).to(_device)
        output = F.grid_sample(inputs, grid, padding_mode='reflection')
...
class HorizontalFlipRandomCrop(nn.Module):
    ...
    def forward(self, input, sign=None, bias=None, rotation=None):
        ...
        grid = F.affine_grid(_theta, input.size()).to(_device)
...
class HorizontalFlipLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        _theta[:, 0, 0] = r_sign
        grid = F.affine_grid(_theta, inputs.size()).to(_device)
        inputs = F.grid_sample(inputs, grid, padding_mode='reflection')
